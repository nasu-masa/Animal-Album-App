...
class RenderDemoSeeder extends Seeder
{
    public function run(): void
    {
         ...
         * 1. コピー開始前に全ソースを検証する
         */
        $files = collect($records)->map(function (array $data): array {
            $relativePath = $data['file_path'];
            $prefix = 'media/seed/';

            if (! str_starts_with($relativePath, $prefix)) {
                throw new RuntimeException(
                    "Seeder用メディアのパスが不正です: {$relativePath}"
                );
            }

            $assetPath = substr($relativePath, strlen($prefix));
            $sourcePath = database_path(
                'seeders/assets/media/'.$assetPath
            );

            if (! File::isFile($sourcePath)
                || ! File::isReadable($sourcePath)) {
                throw new RuntimeException(
                    "Seeder用メディアを読み込めません: {$sourcePath}"
                );
            }
        ...
        /*
         * 2. DBトランザクションの外でファイルを配置する
         */
        foreach ($files as $file) {
            $this->publishFileAtomically(
                $file['source'],
                $file['destination'],
            );
        }
    ...
    private function publishFileAtomically(
        string $sourcePath,
        ...
        // 同時起動しても一時ファイル名が衝突しないようにする
        $temporaryPath = $destinationPath.'.tmp.'.bin2hex(random_bytes(8));

        try {
            if (! File::copy($sourcePath, $temporaryPath)) {
                throw new RuntimeException(
                    "Seeder用メディアのコピーに失敗しました: {$sourcePath}"
                );
            }

            /*
             * 一時ファイルと保存先を同じディレクトリに置くことで、
             * 通常は同一ファイルシステム内の原子的な置換になる。
             */
            if (! @rename($temporaryPath, $destinationPath)) {
                throw new RuntimeException(
                    "Seeder用メディアの配置に失敗しました: {$destinationPath}"
                );
            }
